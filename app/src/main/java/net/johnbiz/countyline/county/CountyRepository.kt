...
/**
 * Owns the (heavy, ~a few MB, load-once) [CountyResolver] built from the bundled
 * `counties.geojson.gz` asset. First access parses the dataset off the main
...
class CountyRepository(private val context: Context) {
    ...
    private suspend fun load(): CountyResolver = withContext(Dispatchers.IO) {
        context.assets.open(ASSET_NAME).use { GeoJsonCountyResolver.fromGzippedGeoJson(it) }
    }

    companion object {
        private const val ASSET_NAME = "counties.geojson.gz"
